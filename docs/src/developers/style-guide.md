...
!!! info
    The style guide is always a work in progress, and not all AbInitioSoftwareBase code
    follows the rules. When modifying AbInitioSoftwareBase, please fix the style violations
...
AbInitioSoftwareBase uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/AbInitioSoftwareBase.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the AbInitioSoftwareBase directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to AbInitioSoftwareBase have
